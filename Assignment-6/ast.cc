...
bool Relational_Expr_Ast::check_ast(){
	...
	fprintf(stderr,"cs316: Error in Relational_Ast\n");
	return false;
}
...
bool Logical_Expr_Ast::check_ast(){
	if(lhs_op!=NULL){
		if(lhs_op->get_data_type() == rhs_op->get_data_type() && lhs_op->get_data_type() != double_data_type){
			return true;
		}
		else{
			fprintf(stderr,"cs316: Error in Logical_Ast\n");
			return false;
		}
	}
	else{
...
bool Selection_Statement_Ast::check_ast(){
	if(else_part!= NULL){
		if(then_part->check_ast() && else_part->check_ast()){
			return true;
		}
	}
	else{
		if(then_part->check_ast()){
			return true;
		}
	}
	return false;
