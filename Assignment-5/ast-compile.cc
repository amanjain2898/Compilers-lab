...
Code_For_Ast & Conditional_Expression_Ast::compile(){
	if(cond->get_data_type()==int_data_type){
		...
		Icode_Stmt *ic_stmt = new Control_Flow_IC_Stmt(beq,opd0,label1);
...
Code_For_Ast & Selection_Statement_Ast::compile(){
	if(cond->get_data_type()==int_data_type){
		...
		string label1 = get_new_label();
		Icode_Stmt *ic_stmt = new Control_Flow_IC_Stmt(beq,opd1,label1);
...
Code_For_Ast & Iteration_Statement_Ast::compile(){
	if(cond->get_data_type() == int_data_type){
		...
		Ics_Opd *opd1 = new Register_Addr_Opd(cond_reg);
		Icode_Stmt *ic_stmt = new Control_Flow_IC_Stmt(bne,opd1,label1);
